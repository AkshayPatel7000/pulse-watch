...
const regions: Region[] = ['us-east-1', 'eu-central-1', 'ap-south-1', 'ap-northeast-1', 'ap-southeast-1'];

export function generateProbeResults(serviceId: string, count: number = 50): ProbeResult[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    regions.forEach(region => {
      ...
      results.push({
        ...
        responseTime: isSuccess ? Math.random() * 500 + 50 : Math.random() * 2000 + 500,
        startedAt: timestamp,
        success: isSuccess,
      });
    });
...
export function calculateUptimeStats(probeResults: ProbeResult[]): UptimeStats {
  const now = Date.now();
  
  const last24h = probeResults.filter(r => r.startedAt > now - 24 * 60 * 60 * 1000);
  const last7d = probeResults.filter(r => r.startedAt > now - 7 * 24 * 60 * 60 * 1000);
  const last30d = probeResults.filter(r => r.startedAt > now - 30 * 24 * 60 * 60 * 1000);
...
export function getRegionalHealth(serviceId: string): RegionalHealth[] {
  ...
  return regions.map(region => {
    ...
    const responseTime = isHealthy ? Math.random() * 300 + 50 : Math.random() * 2000 + 500;
...
class MockDatabase {
  ...
  addService(service: Omit<Service, 'id' | 'currentStatus' | 'lastCheckedAt'>): Service {
